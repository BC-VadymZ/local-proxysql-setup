...
then
  ...
  /usr/bin/mysql -uadmin -padmin -h127.0.0.1 -P6032 -e"
    ...
    INSERT INTO mysql_servers (hostgroup_id,hostname,port,max_replication_lag,comment,max_connections)
      VALUES (0,'mysql-8-primary.default.svc.cluster.local',3306,90,'mysql-master-1',5000);
    INSERT INTO mysql_servers (hostgroup_id,hostname,port,max_replication_lag,comment,max_connections) 
      VALUES (1,'mysql-8-secondary.default.svc.cluster.local',3306,90,'mysql-slave-1',5000);
    ...
    INSERT INTO mysql_users (username,password,default_hostgroup,active) VALUES ('root','root',0,1);
